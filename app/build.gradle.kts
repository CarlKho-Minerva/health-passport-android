...
//    }
    packaging {
        jniLibs {
            // AGP 8.5.1+ default: uncompressed .so, 16 KB zip-aligned APK entries generated by Play.
            // useLegacyPackaging=true forces 4 KB alignment and breaks the Play 16 KB check.
            ...
            // The Nexa SDK loads QNN libs from the assets copies instead of System.loadLibrary.
            excludes += "**/libQnn*.so"
            // libdeepseek-ocr.so has 4 KB ELF alignment (align=0x1000) — fails Play 16 KB check.
            // We don't use DeepSeek OCR (we use PaddleOCR); safe to exclude.
            excludes += "**/libdeepseek-ocr.so"
        }
    }

    androidResources {
        // Keep asset .so files uncompressed so their 16 KB ELF alignment is preserved
        // when Play extracts them. Applies to assets/npu/... QNN runtime libs.
        noCompress += "so"
    }
